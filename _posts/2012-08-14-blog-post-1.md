...
We humans are complicated, imperfect, and a little chaotic ;-). From 2022–2025, models have gotten very good at answering nicely written prompts. But most systems still assume a "clean" Q-A-Q-A loop: I ask a question (Q), the model answers (A), I ask again (Q), it follows the instruction again (A). That’s great for benchmarks and demos; it's not how people actually behave.

As we discussed briefly above, once you stop thinking in terms of neat, turn-based Q-A-Q-A and start thinking in terms of an evolving, closed-loop interaction, a lot of "odd cases" suddenly look like core behaviors. The interesting questions become: How should a model react when users are figuring outthe context at the begging, what happens when we cut into model's reaction mid-stream, and when and who should it be the one to speak up first? In the rest of this section, I’ll group these into three regimes: imperfect, changing, and dangerous/collaborative contexts.
